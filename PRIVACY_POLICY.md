...
- **Speaker separation**: Runs on your device after a recording ends. It stores anonymous labels and timings only; Mityu never attaches a name to a voice and keeps no voice print.
...
- **Live copilot (beta, off by default)**: When enabled under Settings → Beta, the panel follows a recording you start. It holds that meeting's transcript in process memory while you record, writes nothing to disk on its own, and releases the buffer when the recording stops or the copilot is switched off. An answer you request is built from the last few minutes of transcript text after your redaction rules have run, together with the wording of the meeting mode you selected, which is sent as written and is not redacted. Whether it may reach a cloud provider at all is governed by a separate workspace setting that is off by default; the current release offers no way to turn that setting on, so the copilot works only with a model running on this device and its requests do not reach a cloud provider. Were a later release to expose that setting, turning it on would send that text to the provider you configured with your own key. Audio is never involved. The only persistence is a note you pin, written into the meeting's summary as a draft when you save the meeting. With the copilot off, none of this runs: no shortcut, no panel, no buffer.
...
### Models and Updates
Model downloads are started by you and fetch only pinned, integrity-checked model artifacts: transcription and summarization models from Hugging Face, and the speaker-separation models from GitHub (`github.com/k2-fsa/sherpa-onnx` releases). No meeting content is uploaded with those requests. The app may check the Mityu GitHub release endpoint for signed update metadata. Downloaded updater artifacts are accepted only after signature verification.
