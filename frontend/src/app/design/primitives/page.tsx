      ...
      <div className="min-h-screen space-y-8 bg-background p-6 text-foreground">
        ...
        <Group title="Surfaces" hint="§5.6 card · section · well">
          <Card>
            ...
            <CardContent className="space-y-3">
              <CardDescription>
                CardDescription — `text-caption` in `--muted-foreground`, the one-line
                subtitle a card may carry.
              </CardDescription>
              <p className="text-body text-muted-foreground">
        ...
        <Group title="Review rows" hint="§5.10 draft · approved · rejected">
          <SectionCard icon={ListChecks} title="Key points" count={3} aiLabel>
            {/*
              NOT `divide-border`. That utility emits the border-color SHORTHAND on
              `> * + *` at specificity (0,3,0), which beats each row's
              `border-l-verified` / `border-l-destructive` (0,1,0) and repaints the
              §5.10 status rule — the load-bearing "draft vs approved reads from
              across the room" channel — as a neutral grey hairline on every row but
              the first (`+ *` never matches the first child, which is why the draft
              row looked right and hid the bug). `border-t-border` touches ONE side,
              so it cannot collide with a left rule.
            */}
        ...
        <Group title="Notices" hint="§5.17 six tones + the Art. 50 note">
          <div className="space-y-2">
            {/*
              The accessible NAME is per-theme. `Panel` is mounted twice on this route, and
              two `role="note"` elements sharing one name is precisely the DOM shape
              `notice.tsx` says must never occur — the transparency suite's SINGULAR
              `getByRole('note', { name: … })` throws on the second match. The fixture must
              model the invariant it certifies, so the dark copy is named distinctly; the
              PRODUCT renders exactly one banner per surface with the frozen name.
            */}
            <Notice
              tone="ai"
              as="note"
              aria-label={
                theme === 'dark'
                  ? 'AI-generated content, human review required (dark theme sample)'
        ...
        <Group title="Empty states" hint="§5.12 first-run · filtered · blocked">
        ...
        <Group title="Skeletons" hint="§5.19 role=status + sr-only sentence">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="rounded-lg border border-border bg-card p-3">
              <SettingsSectionSkeleton cards={2} />
            </div>
          </div>
