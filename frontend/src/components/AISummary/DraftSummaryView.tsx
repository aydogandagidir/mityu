...
/**
 * 🔒 The four words are unchanged. What changed is that each now carries an ICON as well
 * as a colour, and the colours are tokens: the old chips were twelve raw palette values,
 * so in greyscale, in a printed export, or to a reader with a colour-vision deficiency,
 * "Approved" and "Rejected" were the same grey pill with different words — and this is
 * the surface whose whole job is showing which is which.
 */
/**
 * Secondary row actions: hidden to the eye until the row is hovered or something inside
 * it has focus, and unconditionally visible where there is no hover at all. Never
 * `display:none` and never `pointer-events-none` — both would take them out of the tab
 * order, which is how a "reveal on hover" pattern quietly becomes mouse-only.
 */
const REVEALED_ACTIONS = [
  'flex items-center gap-1 opacity-0 transition-opacity duration-fast ease-out',
...
function ReviewRequiredBanner() {
  // 🔒 `role="note"`, the accessible name and both sentences are unchanged, and it is
  // still rendered in all four states with no way to dismiss it. Two things changed:
  // it is the `Notice` primitive, which DROPS any element passed as a child rather
  // than rendering it — so "contains no control" is a property of the component and
  // not a promise a future edit can quietly break — and it wears the AI register
  // rather than amber. Amber is this app's warning colour; a draft is not an error,
  // and a marking that looks like an error is one a reader learns to dismiss.
  return (
    <div className="mb-4">
      <Notice
        tone="ai"
...
/**
 * The jump-to-evidence control. 🔒 Its accessible name is unchanged.
 *
 * It used to render NOTHING when `onJumpToSource` was absent — so a parent that forgot
 * one prop silently removed the source link from every block, and the summary looked
 * exactly as complete as one whose evidence resolved. The chip is now always drawn: a
 * caller with no handler gets the unresolved state, which says so in a sentence.
 */
function SourceLink({
  sourceChunkId,
  onJumpToSource,
  ...
  return (
    <SourceChip
      resolved={!!onJumpToSource}
      onClick={onJumpToSource ? () => onJumpToSource(sourceChunkId) : undefined}
      title="Jump to the source transcript segment"
    />
  );
}
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          {isEditing ? (
            <textarea
              ...
              className="w-full resize-y rounded-md border border-input bg-background px-3 py-2 text-body text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-card"
              autoFocus
            />
          ) : (
            <p
          ...
          {isRejecting && (
            <input
              ...
              className="mt-2 w-full rounded-md border border-destructive-border bg-background px-3 py-2 text-body text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-card"
              autoFocus
            />
          )}
        </div>
        ...
        <div className="flex-shrink-0 flex items-center gap-1">
          {isEditing ? (
            ...
            <>
              {/* Approve is the decision this screen exists for, so it keeps its WORD.
                  A row of three identical icon buttons made the primary action a
                  guess, and the only thing distinguishing "approve" from "reject" was
                  a tick against a cross at 16px. */}
              <Button
                variant="outline"
                size="row"
                onClick={onApprove}
              ...
              {/* Edit and Reject reveal on hover or keyboard focus and are ALWAYS
                  visible on a coarse pointer, where there is no hover. They stay in
                  the DOM and in the tab order in every state: `opacity-0` hides them
                  from the eye, never from the keyboard. */}
              <div className={REVEALED_ACTIONS}>
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={startEdit}
...
export function DraftSummaryView({
  ...
  if (error) {
    return (
      <div className="w-full">
        <ReviewRequiredBanner />
        <div className="rounded-md border border-destructive-border bg-destructive-surface p-4 text-destructive-ink">
          <p className="text-sm text-red-700 dark:text-red-300 font-medium">
            Couldn&apos;t load the summary draft
          </p>
          <p className="mt-1 text-body">{error}</p>
        </div>
      </div>
    );
  }
