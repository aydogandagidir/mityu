...
/**
 * Dropdown menu — `--popover` + hairline + `--elev-1` (§4.8), items on `--accent`.
 *
 * Menu items keep `outline-none`, and that is correct here: Radix roving focus means exactly
 * one item is focusable at a time and the `--accent` highlight already tracks it, so a ring
 * would double-draw on every arrow press. `HIGHLIGHT_ITEM` is that replacement, written once
 * (see `focus-ring.ts`) so "stripped the outline and replaced it with nothing" — the defect
 * in 16 shipped files — cannot recur silently.
 */
 ...
 */
const menuPanel = cn(
  "z-70 min-w-[8rem] overflow-hidden rounded-md border border-border bg-popover p-1 text-popover-foreground shadow-elev-1",
