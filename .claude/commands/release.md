...
5. Package + sign every platform in the approved matrix: Tauri updater plus platform signing/notarization as applicable. For Windows-only v1.0.4 this means the **updater signature only** — Windows Authenticode is deferred (ADR-0029), matching the v1.0.0–v1.0.3 line, so installers show the SmartScreen unknown-publisher prompt; signing secrets come from CI/keychain and are never committed.
