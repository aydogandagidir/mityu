...
1. Delete that clip's `.wav`, `.ref.txt` and any `.draft.txt`.
...
The verifier then inverts its checks for that row: it requires the artifacts to be **absent** rather than present, and stops counting the row toward the ≥5-per-bucket gate. If that drops a bucket below five, verification **fails** — which is the intended behaviour. A gate must not be closed with a four-clip bucket because someone exercised a right; either record a replacement clip or report the bucket as incomplete.
