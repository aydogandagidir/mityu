...
// MITYU PATCH: `piper_phonemize`, `espeak-ng` and `ucd` removed.
//
// eSpeak NG is GPL-3.0. Upstream's default archives are built with
// `SHERPA_ONNX_ENABLE_TTS=ON`, and this list made the linker pull espeak's
// object code into every binary — measured, not assumed: the release
// `diarize-helper` carried 51 espeak symbols, and `/OPT:REF` did not discard
// them. Shipping that inside Mityu's closed installer would distribute GPL-3.0
// code without GPL terms or corresponding source.
//
// This list is not guesswork: it is exactly the set of `.lib` files present in
// the `-no-tts-lib` archive this crate now fetches. Diarization uses no TTS.
...
fn archive_name(
    ...
    let name = match (link_mode, target_os, target_arch) {
        // MITYU PATCH: every static target now asks for the `-no-tts` archive,
        // which upstream publishes alongside the default one. See the note on
        // SHERPA_ONNX_STATIC_LIBS — the default archives carry GPL-3.0
        // espeak-ng, and these do not.
        (LinkMode::Static, "linux", "x86_64") => {
            format!("sherpa-onnx-v{version}-linux-x64-static-no-tts-lib.tar.bz2")
        }
        // MITYU PATCH: upstream publishes NO `no-tts` build for this target —
        // every other pinned target has one. Failing here is deliberate: the
        // alternative is silently linking GPL-3.0 espeak-ng on aarch64 Linux,
        // which neither `ci.yml` nor `build.yml` builds and which ADR-0022
        // already records as having broken system audio.
        (LinkMode::Static, "linux", "aarch64") => {
            return Err("aarch64 Linux has no upstream sherpa-onnx `no-tts` build; \
                        building it would link GPL-3.0 espeak-ng (see BACKLOG H10)"
                .into())
        }
