...
14. ~~Zero frontend tests~~ — **substantially resolved.** Vitest is wired into CI; the v1.0.4 candidate passes 14 files / 126 tests covering export approval/provenance/sanitization, IndexedDB workspace isolation, recording-consent IPC, context-bound completion/retry/recovery and review-state logic. **Still open:** browser-level component/onboarding/settings coverage remains limited; the suite is not a substitute for the signed installer smoke or C8 human pilot.
