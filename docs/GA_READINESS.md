...
7. **v1.0.4 protected PR/exact-SHA CI is active and green.** `origin/main` is the published, rebranded Mityu v1.0.3 commit (`0a503be`), not vanilla upstream. Release PR #4 contains the v1.0.4 work; its required `rust`, `frontend` and `server-isolation` checks passed on implementation commit `ae09449b8da0c60968cda0ed38e87c8c444a1bb9`. Branch protection must keep the current PR head green before merge.
