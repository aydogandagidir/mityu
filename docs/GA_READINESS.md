...
5. **Supply chain partially resolved.** ~~Parakeet model~~ now downloads from `huggingface.co/istupakov/parakeet-tdt-0.6b-v3-onnx` instead of `meetily.towardsgeneralintelligence.com` (fixed 2026-07-07, ADR-0020). **Still open**: FFmpeg binaries from `github.com/Zackriya-Solutions/ffmpeg-binaries` — a product that states "not affiliated with Meetily" depending on Meetily's own servers at every install remains a reliability risk and a branding inconsistency for this one remaining dependency.
