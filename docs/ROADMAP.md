...
- **Core is integration-free (by design):** Mityu captures system audio ("works everywhere … online or offline") and is **not a meeting bot by default** (`CLAUDE.md` §1). Meet/Zoom/Teams-related capabilities exist only as **opt-in, consent-gated integrations** — calendar metadata (light) and, in Phase 2+, a call-joining bot — per **ADR-0018** (BACKLOG EPIC G, post-C8).
