...
**v1.2.1 validation scope (ADR-0045/0046, on the same terms as v1.2.0):** A5 and C8 remain `DEFERRED / NOT PASSED`. v1.2.1 is a PATCH: it ships two things and no new capability claim. First, the copilot's on-switch and the Modes editor are now actually reachable, under Settings → Beta (ADR-0045) — in v1.2.0 they were mounted in a component nothing rendered, so the whole of EPIC I was invisible; nothing about the I9 gate moved and `liveCopilot` is still off by default. Second, "Pin to notes" (I3c, ADR-0046): a kept live answer becomes a `pinned` draft block that survives regeneration, written only when the meeting is saved and never able to fail the save. The `summaries.sections` JSON gains an optional `provenance` field; every stored blob reads as `generated`, no migration. Unverified rather than passed: the pin path end to end on a real build (press Pin → stop → save → open the summary) is covered by tests, not by a human; the model call is still unexercised against a real provider (I9). The §2c Polar pin check remains unverified for the same reason as v1.2.0. Everything below still applies.

**v1.2.0 validation scope (ADR-0043, extending ADR-0027/0037 on the same terms):** A5 and C8 remain `DEFERRED / NOT PASSED` for v1.2.0 publication. v1.2.0 adds the Live Copilot (EPIC I: panel, live context, grounded on-demand insights, meeting modes) and makes **no accuracy claim of any kind**. It ships **off by default** — with `liveCopilot` off the build behaves exactly like one compiled without the module — which is what makes publishing it before the I9 gate honest. Three pre-release security fixes landed with it (a remote Ollama endpoint defeating the egress policy; unbounded mode fields able to inject a replacement prompt contract; redaction failing open on a settings read error); see ADR-0043 for those and for the accepted residuals. Unverified rather than passed: the §2c Polar pin check, which this session's egress proxy refused. The model call has never been exercised against a real provider — that remains the I9 live smoke. Everything below still applies.
...
**Public copy per release: there is nothing to edit, by design (ADR-0074).** The landing page names no version and lists no releases. Release history is published per release on GitHub and the footer links there; the download endpoint resolves the current release from `latest.json` at request time, so the site describes the newest build without being touched. `landing/tests/claims.test.mjs` enforces this: it fails CI on any hard-coded `x.y.z` in the page, on changelog or roadmap markup returning to it, and on the copilot being described as anything other than what the shipped build does. What DOES need a per-release edit is the in-app note below. Capability or beta wording that changes — the copilot ceasing to be a beta, say — must be updated in three places at once: `landing/index.html`, `landing/privacy.html` and `frontend/src/components/About.tsx`, whose own test pins the shared phrases.

**Re-key the in-app release note to the version it actually ships as.** `frontend/src/lib/releaseNotes.ts` (ADR-0071) is keyed by version, and `whatsNewFor` returns only notes **strictly newer** than the user's last-seen version. A note written under a version that is already published therefore reaches nobody: a user on published v1.2.1 who updates to v1.2.2 is shown notes `> 1.2.1`, so a `1.2.1` entry describing work that landed *after* that tag is silently skipped, and the release it describes is announced to no one. Before dispatching a release, check that the top `RELEASE_NOTES` entry's `version` equals the version being cut — not the one it was drafted against — and that its `caveats` still hold. This is now the only per-release copy task, and nothing else cross-checks it: the site-versus-app version check was removed with the changelog section it depended on, so the review of this entry is the check.
...
1. **Deferred for v1.0.4 (ADR-0029):** the updater secrets stay repository-scoped and are read by the `Production`-environment jobs, so this migration is **not** required to ship v1.0.4 (which is updater-signed only). When later performed: recover the existing private-key password, upload `TAURI_SIGNING_PRIVATE_KEY` and `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` into `Production`, verify a successful environment build, and only then delete the repository-scoped copies. Do not generate a replacement key: that would break updater continuity for v1.0.3 installations. Non-production workflows disable updater artifacts and cannot consume the production key.
2. **Pre-publication:** inspect the draft artifacts and require the production build's `*.sig` files, exact versioned `aydogandagidir/mityu` NSIS URL/signature in `latest.json`, and a valid updater key chain. The release workflow performs these fail-closed checks against the unpublished draft, including cryptographic verification of the remotely downloaded NSIS artifact with the public key baked into the exact release commit. **v1.0.4 is updater-signed only — Windows Authenticode is deferred (ADR-0029), so no publisher signature/timestamp is expected.**
