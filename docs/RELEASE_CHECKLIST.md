...
Both dependencies are resolved for Windows/Linux; FFmpeg on macOS remains open:
- **FFmpeg** — Windows and Linux (x64/arm64) ~~fetched from `github.com/Zackriya-Solutions/ffmpeg-binaries`~~ **fixed (2026-07-07)**: `frontend/src-tauri/build/ffmpeg.rs` now downloads from a self-hosted release, `github.com/aydogandagidir/mityu` tag `ffmpeg-deps-8.1-lgpl`. This also switched the build from GPLv3 (the original gyan.dev-derived binary had `--enable-gpl`, confirmed via its own `-version` banner) to **LGPL-only** static builds mirrored from `BtbN/FFmpeg-Builds` — Mityu never uses a GPL-only codec (audio-only `-c:a aac`, no `-c:v` anywhere), so this avoids the GPL source-redistribution obligation entirely (see ADR-0021). **macOS (Intel + Apple Silicon) still fetches from the Zackriya mirror** — no equivalently-licensed LGPL-only static macOS build has been sourced yet.
  > ⚠ **Blocking check before the first signed release:** the new Windows/Linux binaries were verified structurally (archive layout, valid PE32+, asset sizes) but **never executed** — the `ffmpeg -version` check that would confirm the LGPL config was blocked by tooling safety controls and is still owed. Delete the cached `frontend/src-tauri/binaries/ffmpeg-*` file, run one real `cargo build`, and confirm the fresh banner shows **no** `--enable-gpl` and **no** `gyan.dev`. A routine build reuses the stale cache and prints a *passing* verification line for the OLD GPL binary — that line is not evidence.
