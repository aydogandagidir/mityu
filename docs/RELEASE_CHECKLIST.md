...
**Public copy per release.** The landing page (`landing/index.html`, section `#whats-new`) lists releases by version in `data-release` attributes, and the desktop About dialog (`frontend/src/components/About.tsx`) mirrors its feature and beta wording. Add the new release's entry — and any changed capability or beta wording — once its signed release exists (a follow-up PR right after publication is fine), and keep the two surfaces saying the same thing. `main` is git-connected to Vercel, so merging publishes the site: never list a version there before it can be downloaded. `landing/tests/whats-new.test.mjs` fails the CI gate if the list runs ahead of `frontend/package.json`, or falls more than one minor behind it. The FAQ and pricing footnote are deliberately version-agnostic so only the `#whats-new` list needs a per-release edit.

**Re-key the in-app release note to the version it actually ships as.** `frontend/src/lib/releaseNotes.ts` (ADR-0047) is keyed by version, and `whatsNewFor` returns only notes **strictly newer** than the user's last-seen version. A note written under a version that is already published therefore reaches nobody: a user on published v1.2.1 who updates to v1.2.2 is shown notes `> 1.2.1`, so a `1.2.1` entry describing work that landed *after* that tag is silently skipped, and the release it describes is announced to no one. Before dispatching a release, check that the top `RELEASE_NOTES` entry's `version` equals the version being cut — not the one it was drafted against — and that its `caveats` still hold. `landing/tests/whats-new.test.mjs` asserts every version in `releaseNotes.ts` is also listed on the site, which catches a note keyed to a version that was never released.
