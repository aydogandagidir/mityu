...
- Options, in the order worth trying: (1) check whether k2-fsa publishes a TTS-free prebuilt asset; (2) build sherpa-onnx from source with `SHERPA_ONNX_ENABLE_TTS=OFF` into the same archive layout and point `SHERPA_ONNX_ARCHIVE_DIR` at it — the indirection H2 already built makes this the natural fit, but the pin then covers *our* artifact, so ADR-0035's "pinned upstream release" story and H2's integrity bootstrap both need rewriting; (3) patch/fork `sherpa-onnx-sys` for the link list; (4) a different engine. **Owner decision — this changes ADR-0035.**
