...
### I3a · On-demand insights — the Rust pipeline (no UI) — ✅ BUILT 2026-09-13 (`feat/i3a-insight-pipeline`; PR pending)
...
### I3b · On-demand insights — the panel UI
- Agent: frontend-nextjs-engineer · Cmd: /feature · depends-on: I3a
- AC: four actions by button or hotkey; a Tauri command over `copilot::insight::generate_insight`; streamed cards with cancel; the panel shows the Art. 50(1) first-open disclosure ("You are interacting with an AI assistant") and every card carries a non-dismissable "AI-generated · verify before you rely on it" marking; `NoContext`, `Refused` and `CloudNotAllowed` each render as themselves, never as an empty card; nothing is persisted unless the user presses **Pin to notes**, which writes a `draft` block with `source_chunk_id` into that meeting's structured summary draft through the existing repository (status forced `draft`, the C1 rule); insight and window text never reach logs or analytics; works fully offline with the built-in model.
- Verification: Vitest for card states and marking non-dismissability (the ADR-0032 mutation pattern); a network-OFF smoke of all four actions.

### I4b · Modes UI (panel selector + Settings editor)
- Agent: frontend-nextjs-engineer · Cmd: /feature · depends-on: I4a, I3b
