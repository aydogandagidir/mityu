...
### I3c · Pin to notes — ✅ BUILT 2026-09-14 (`feat/i3c-pin-to-notes`; PR pending)
- Agent: rust-tauri-core-engineer · Cmd: /feature · depends-on: I3b
- Was blocked on a data-model decision, now recorded as **ADR-0046** (with `docs/CONTRACTS.md` §4 updated and the sync-compatibility note written, as the blocking condition required).
- Why it was deferred, verified in the code and all three now addressed: (1) there is **no `meetings` row** during a recording → pins buffer in memory in `copilot::pin` and flush at save time; (2) a pinned block's `source_chunk_id` must resolve to a live `transcripts` row but the copilot's ids are in-memory `t{sequence_id}` → the bridge turned out to already exist in flight: `TranscriptContext` accumulates `sequence_id`, dedupes and sorts by it, and `saveMeeting` passes it through — `api::TranscriptSegment` simply had no field for it, so serde dropped it. It now carries `sequence_id: Option<u64>` and `save_transcript` returns the `sequence_id → transcripts.id` map it minted. **No migration**; the map is consumed by the flush. Float-matching `audio_start_time` was considered and rejected (it mis-attributes a citation when two segments start close together, and a wrong timestamp under a claim is worse than none); (3) `upsert_draft` bound `sections = ?` wholesale, so the ordinary post-meeting summary **erased every pinned block** → `DraftBlock` gains `provenance: generated|pinned` (`#[serde(default)]`, no backfill) and `upsert_draft` now reads the row inside its own transaction, lifts out the pinned blocks and carries them into a trailing "Pinned during the meeting" section.
- AC: `copilot::pin` holds the session's pins — **a separate buffer from `LiveContext`, and that is the point**: `recording-stopped` resets the live context, but the save that persists a pin runs *after* the stop, so a pin living in the context would be cleared moments before the flush read it and the feature would pin nothing while appearing to work. Cleared on `recording-started` and when the copilot is switched off, never on stop. Bounded at 100 with a stated refusal. `SummariesRepository::append_block` writes over the existing private CAS writer (no second `sections = ?` writer) and **creates the summaries row when none exists**, which is the common case since pins land before generation. Status forced `draft`; evidence must resolve at write time; only this path may mint `pinned`. Preserved pinned blocks keep their own status — forcing them back to `draft` would revoke a human approval on every regenerate — while incoming generated blocks are forced as before. Carried-across pins are deliberately **not** re-validated: a retranscription that removed a cited segment would otherwise make generation impossible, and dropping them would destroy user-kept content on an unavoidable path; approval re-checks evidence, so such a block survives and simply cannot be approved. `copilot_pin_insight`/`copilot_unpin_insight`; status carries `pinnedClaimIds` + `pendingPins` so the panel renders pinned state from the backend rather than from its own memory. **A pin can never fail a save**: the flush runs after the commit and every loss is a counted, content-free report shown to the user.
- Verification: **the test ADR-0041 demanded** — a full generate-after-pin cycle plus three regenerates asserting the pinned block survives while the generated half is replaced — and 8 more repository tests (human verdict preserved; dead source neither blocks generation nor disappears; a generator cannot mint `pinned`; unresolvable citation refused; status forced; pins accumulate in one section; cross-workspace refused without creating a row). 8 `copilot::pin` tests (serialised on the shared static, as `session`'s are) and 6 `copilot::flush` tests including "an unresolvable pin is not attached to a neighbour". Strict `sequence_from_passage_id` with 10 rejection cases. **Seven mutations applied and killed**: preservation removed, preserved pins forced to draft, generator allowed to mint `pinned`, carried-across pins re-validated, `append_block`'s evidence gate removed, its status forcing removed, its workspace check removed. `cargo test --lib` 685+ pass; tsc/lint clean; `pnpm vitest run` with 9 new pin cases in `CopilotInsights.test.tsx`.
- Residual, stated rather than hidden: a pin made during a recording the user then abandons is lost with the session buffer, because there is nothing to attach it to — the panel says pins are written when the meeting is saved. The `Pin` and `Unpin` labels are not in the I9 live smoke's scope; what a model returns is still unverified (ADR-0043).
...
- **Correction (ADR-0045):** I1 and I4b mounted `CopilotSettings` and `ModesSettings` in `SettingTabs.tsx`, a component nothing in the app renders — the real Settings page is `app/settings/page.tsx` with its own tab list. So in v1.2.0 the switch that turns the copilot on could not be reached, and the whole epic was invisible. Both now live as cards in **Settings → Beta**, where a pre-gate feature belongs; `BetaSettings.test.tsx` pins the mounting and was mutation-checked both ways.

### G · Post-v1.2.1 owner reports — ✅ BUILT 2026-09-15 (`fix/recording-start-engine-aware`; PR pending)
- Agent: rust-tauri-core-engineer + frontend-nextjs-engineer · Cmd: /fix-bug then /feature · ADR-0047
- **G1 — recording would not start (the core path).** The pre-flight in `useRecordingStart.ts` asked `parakeet_has_available_models` on all three start paths regardless of the configured engine, so a Local Whisper user was refused 100% of the time while the Rust check on the same path would have accepted. **Not a regression:** the file is byte-identical between the v1.2.0 tag and HEAD, and nothing on the start path is in that diff. Replaced by one engine-aware Rust command `api_transcription_readiness` whose pure core `decide_readiness` is unit-tested; the renderer renders its sentence and re-decides nothing (ADR-0042). A pre-flight that cannot run reports ready rather than blocking.
- **G1b — the silence that made it unreportable.** `tauri-plugin-log` was declared only for macOS, so the Windows build linked no log crate and wrote no log file anywhere while `windows_subsystem = "windows"` discarded stdout; two competing `env_logger` inits (one in a registered command that would have panicked) are gone and the plugin is registered for every platform. Also fixed: consent cancels that returned to idle with only a `console.log`, two `alert("check console")` in a build with no console, the backend announcing "the model is still downloading" for every validation failure, `RecordingControls` discarding the backend's reason for canned device advice, and `recording-error` — emitted by Rust, listened to by nobody.
- **G2 — what's new.** Bundled `releaseNotes.ts` (no fetch; local-first), shown once per version keyed by a last-seen version in the Tauri store, silent on a fresh install, re-openable from Settings → General. Caveats ship in the same dialog as the features.
- **G3 — the copilot was reachable in theory only.** Tray entry that toggles the panel when on and routes to `/settings?tab=beta` when off; the **missing `allowCloudInsights` switch** (enforced on every request, settable nowhere — the feature was untestable for anyone on a cloud provider); three shipped strings naming a "Settings → Copilot" tab that never existed; the panel footer still claiming "Nothing it offers is saved" after ADR-0046 made pinning persist; the shortcut hint hidden exactly when the OS refused the binding.
- **G4 — density.** Measured first: Settings → General carried ~592 words across nine identically-weighted cards, while every legal and honesty marking in the app totals under 200 words — so the problem was structure, not compliance text. Added a five-step type scale (there was none: eight sizes plus arbitrary `[10px]`/`[11px]`/`[13px]`), a `SettingCard` primitive with a native `<details>` slot, rebuilt Settings → General on it, fixed four *invalid* Tailwind classes in `About.tsx` that silently fell back to inherited sizes, and deleted dead markup. **Folding an explanation shortens it and never removes it**: the text stays in the DOM, find-in-page reaches it, it opens with no JavaScript. Legal markings are out of scope for folding.
- Verification: 7 `decide_readiness` tests (including the inverted case that was the bug), 9 start-path guard tests, 12 release-note selection tests, 7 copilot-settings tests, 7 `SettingCard` tests; 8 mutations applied and killed (Parakeet gate reintroduced, our own message composed over the backend's, a failed pre-flight blocking, a silenced consent cancel, and four on the UI primitives). `cargo test --all` green; tsc, lint and `pnpm vitest run` 293 pass.
- CI (ADR-0048): the five manual build wrappers had **never completed a run** — `build.yml` demanded `contents: write` from every caller and only `release.yml` gave it, so each was an invalid workflow (`startup_failure`, zero jobs). `build.yml` now inherits the caller's permissions; the wrappers work with a read-only token. And `ci.yml` runs the **full Windows release build on every native pull request**, uploading `mityu-pr-x86_64-pc-windows-msvc` (.msi + NSIS .exe) so a PR can be smoke-tested without a local build. Docs-only PRs skip it; releases never run it twice.
- Not done, queued rather than implied: the remaining density stages (~115 hardcoded `gray-*`/`bg-white` classes that break dark mode, the two identically-styled AI markings stacking in one panel, the `docs/DESIGN_READAI.md` Phase A token migration). **Nothing here was seen running** — there is no Windows machine in this environment; the owner's smoke test is what confirms it.

### H · Verification: start the program and look — ✅ BUILT 2026-09-16 (`fix/startup-verification`) · ADR-0049
- Asked directly: what is needed to verify epic G, and go get it. Probing the container settled which limits are real: **Xvfb + webkit2gtk + Chromium are present, so the desktop app builds and launches here**; `/dev/snd` is absent, so capture genuinely cannot be exercised anywhere in CI.
- Launching it found **two defects no test in this repo could find, both in code epic G wrote**. (1) Every log line written **twice, into two byte-identical files** — `Builder::target()` APPENDS to `DEFAULT_LOG_TARGETS`, so two `.target()` calls made four targets; proven by one PID holding two write fds, both files 113513 bytes, same md5. The 5 MB `KeepOne` rotation was applied per file to two files. Same defect class as the bug G fixed, introduced by the fix for it. (2) `/settings?tab=beta` threw a **React hydration error** — the tray deep-link read `window.location.search` in a `useState` initialiser, and the app is a static export, so the prerender could only ever say General.
- Built: `tools/ci/verify-startup.sh` — launches the binary, waits for the app's own `Application setup complete`, asserts **exactly one** log file with real content. Wired into CI's existing `rust` job (warm cache, no new job). **Mutation-tested before being trusted**, per this repo's `--self-test` convention: bug reintroduced → script FAILS naming both files; fix restored → PASSES.
- Also fixed: `tools/ui/shoot.py` now passes `--no-sandbox` as root. Chrome exits 1 rendering nothing otherwise, so the repo's own screenshot gate could not run in any container or CI runner.
- Honest limits: the check runs on ubuntu, so it proves the log plugin is no longer macOS-gated — **not** that the Windows build logs (different log dir, `windows_subsystem = "windows"` kills the stdout target). **CLAUDE.md §4 still owes a human record→transcript smoke test.**
- Queued, not done: `tauri = { features = ["test"] }` for a real `AppHandle` on Linux, which would close the three largest untested claims (`api_transcription_readiness`'s config wrapper, the `transcription-error` payload shape, the tray copilot handler — `tray.rs` has zero test blocks); replacing the remaining source-text assertions with jsdom renders.

### I · The three silent refusals epic G did not close — ✅ BUILT 2026-09-17 (`fix/silent-recording-refusals`) · ADR-0050
- The owner reported v1.2.1 **still** will not record. An adversarial pass over the SHIPPED code — assuming the engine-blind pre-flight was *not* the cause — found three more gates of the same family: **state a failure leaves behind that nothing clears and nothing explains.**
- **One unfinished save refused every later recording for the life of the process.** The guard was `!pending.is_empty()`; the renderer acknowledges at the END of its success path, so any earlier failure left the entry forever. No user action cleared it — closing the window only *hides* the app, so what a user calls a restart is not one. Now time-bounded by `PENDING_SAVE_GRACE` (120s, past the renderer's own 60s wait): inside it the guard still refuses; past it the save is stuck, not running. **The entry is kept** — folder still authoritative for recovery, token still claimable (pinned by a test) — and letting a start through logs a `warn!`.
- **A failed stop left `IS_RECORDING` true with no manager behind it**, so the tray and the UI showed a recording that did not exist and every start was refused with "Recording already in progress". The flag is now cleared the moment the manager leaves the global — the point of no return — so no later early return can outrun it.
- **No microphone meant no button at all** — an empty strip, no control, no explanation. A `NoMicrophoneNotice` now takes the slot, separating "no input device" from "could not read the device list", naming the Windows privacy setting, and offering **Check again** instead of a restart. Plus: one early `return` in `handleRecordingStop` bypassed every re-enable; the reset moved into the `finally`.
- Verification: 5 new Rust tests + 5 new component tests; **mutation-tested** — the old `!is_empty()` gate fails two tests, collapsing the notice's two failure modes fails one. `cargo test --lib` green, vitest 308 pass, tsc/lint clean.
- **Not tested, stated not implied:** the `IS_RECORDING` fix sets a process-global inside an async Tauri command needing a real `AppHandle` — it rests on reading until the queued `tauri = { features = ["test"] }` work lands. The notice's wiring into `page.tsx` is untested; only the component is. **Nothing here was seen running on Windows.**
