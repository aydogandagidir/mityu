...
### I2 · Live context service (deterministic, offline)
- Agent: rust-tauri-core-engineer · Cmd: /feature · depends-on: I1
- AC: `copilot::session` subscribes to `transcript-update` with `app.listen` (the `recording_commands.rs:323` pattern) and keeps a bounded rolling window (default 180 s, configurable) plus a durable per-session buffer; turns are attributed me/them from `source`; partial results (`is_partial`) are display-only and never enter the durable buffer, a final replaces its partial; a deterministic cue detector (TR + EN interrogatives, punctuation, addressing cues) emits `copilot-cue` events carrying a confidence and the `sequence_id`s of the evidence segments; no LLM call, no I/O, no persistence; tenant-scoped through `AuthContext`; with the flag off nothing subscribes.
- Verification: unit tests for TR/EN cues, partial→final replacement, window eviction, no cue from `microphone` turns, and zero subscriptions with the flag off.

### I4a · Modes registry v1 (minimal; parallel with I2)
...
- Verification: validator and defaults unit tests; a test that every built-in mode resolves to an existing summary template.

### I3 · On-demand insights v1 — Suggest · Follow-up questions · Recap · Define
- Agent: rust-tauri-core-engineer + frontend-nextjs-engineer · Cmd: /feature then /security-review · depends-on: I2, I4a
- AC: four actions, by button or hotkey; the prompt is the mode fragment (I4a) + the **redacted** rolling window (`redaction::redact`) + knowledge passages when I5 exists; the model is reached only through the existing `summary/` provider layer (BYOK; built-in local model is the default; a per-workspace "cloud allowed for live insights" policy is honoured — MULTITENANCY per-tenant policy seam); output is typed JSON with a per-claim segment reference; `ask::grounding` rules reused — an uncited or out-of-window citation is dropped, never repaired; empty window ⇒ a refusal outcome, never an empty answer; streamed to the panel with cancel + timeout; the panel shows the Art. 50(1) first-open disclosure ("You are interacting with an AI assistant") and every card carries a non-dismissable "AI-generated · verify before you rely on it" marking; nothing is persisted unless the user presses **Pin to notes**, which writes a `draft` block with `source_chunk_id` into that meeting's structured summary draft through the existing repository (status forced `draft`, the C1 rule); insight and window text never reach logs or analytics; works fully offline with the built-in model.
- Verification: Rust tests for prompt assembly, redaction-before-provider and grounding; Vitest for card states and marking non-dismissability (the ADR-0032 mutation pattern); a network-OFF smoke of all four actions.

### I4b · Modes UI (panel selector + Settings editor)
- Agent: frontend-nextjs-engineer · Cmd: /feature · depends-on: I4a, I3
- AC: mode chip in the panel; Settings → Modes lists built-ins and custom modes, imports a custom mode file with the I4a preview, edits name/purpose/voice/sections; no raw `invoke` in components (`services/modesService.ts`).
